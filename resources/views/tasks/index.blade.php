            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <h1><strong>Crear Nueva Tarea</strong></h1>
                    ...
                    <form action="{{ route('tasks.store') }}" method="POST">
                        ...
                        <textarea id="description" name="description"></textarea><br> <!-- {{ old('description') }} para mantener los datos introducidos con anterioridad -->
                        ...
                        <select id="assigned_to" name="assigned_to" required>
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}">{{ $user->name }}</option> <!-- usuario selecciona a que usuario asignar la tarea -->
                            @endforeach
                        </select>
